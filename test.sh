...
python code/get_sdd_splits.py resized_videos.lst data_splits --n_fold 1
...
python code/evaluate_sdd.py resized.lst sdd_out.p

echo $(echo $PWD | sed 's/^\/c/C:/')/sdd_out.p,0_0_255 > sdd_run.lst

python code/visualize_output.py sdd_run.lst resized_videos_frames \
sdd_vis_heatmap/ --vis_num 100 --use_heatmap

python code/merge_frames.py sdd_vis_heatmap --fps 5